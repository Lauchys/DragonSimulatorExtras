...
public class DemoCommand extends AbstractCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(Utils.c("&cUsage: /demo <player>"));
            return true;
        }
        if (args.length == 1) {
            // Sends to all online players, including yourself lol
            if (args[0].equalsIgnoreCase("-a")) {
                for (Player online : Bukkit.getOnlinePlayers()) {
                    online.sendMessage(Utils.c("&eAn &cAdmin &eis messing with the game settings..."));
                    player.sendMessage(Utils.c("&aSuccess! &2All online players &ahave been sent to the demo screen"));
                    Utils.sendDemo(online);
                    return true;
                }
            }
            ...
            player.sendMessage(Utils.c("&aSuccess! &2" + target.getName() + " &ahas been sent to the demo screen"));
            Utils.sendDemo(target);
            return true;
        }
        return true;
    }
